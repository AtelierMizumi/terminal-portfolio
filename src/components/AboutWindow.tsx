...
import { Github, Globe, Mail, ExternalLink, Keyboard, Cpu, Code } from "lucide-react";
        ...
        <div className={`h-full overflow-y-auto bg-[#0d0d0d] text-gray-200 ${className}`}>
            ...
            <div className="px-6 py-4 border-t border-gray-800/50">
                ...
                <p className="text-sm text-gray-300 leading-relaxed">
                    A Computer Science student passionate about <span className="text-purple-400">Linux ricing</span>,
                    building <span className="text-cyan-400">custom keyboards</span> with ZMK firmware,
                    and crafting beautiful <span className="text-green-400">web experiences</span>.
                    I run CachyOS with KDE Plasma and enjoy tinkering with NixOS configurations.
                </p>
            </div>
            ...
            <div className="px-6 py-4 border-t border-gray-800/50">
                ...
                <div className="grid grid-cols-2 gap-2">
                    {[
                        { icon: <Code className="w-4 h-4" />, label: "Web Development", color: "text-green-400" },
                        { icon: <Keyboard className="w-4 h-4" />, label: "Custom Keyboards", color: "text-cyan-400" },
                        { icon: <Cpu className="w-4 h-4" />, label: "Embedded Systems", color: "text-yellow-400" },
                        ...
                        <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.03] hover:bg-white/[0.06] transition-colors">
                            <span className={item.color}>{typeof item.icon === "string" ? item.icon : item.icon}</span>
            ...
            <div className="px-6 py-4 border-t border-gray-800/50">
                ...
                <div className="space-y-2">
                    {[
                        { name: "Terminal Portfolio", desc: "This! Virtual desktop with Linux rice aesthetics", url: "https://github.com/AtelierMizumi/terminal-portfolio", tag: "Active" },
                        { name: "Personal Blog", desc: "Jekyll + Chirpy theme on GitHub Pages", url: "https://ateliermizumi.github.io", tag: "" },
                        { name: "ZMK Firmware", desc: "Custom firmware for split keyboards with OLED", url: "https://github.com/AtelierMizumi/zmk-firmware", tag: "" },
                        { name: "NixOS Config", desc: "Personal NixOS + KDE Plasma configuration", url: "https://github.com/AtelierMizumi/nixos-kde-config", tag: "" },
                    ].map((project) => (
                        <a
                            key={project.name}
                            href={project.url}
                        ...
                        >
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 truncate mt-0.5">{project.desc}</p>
                            </div>
