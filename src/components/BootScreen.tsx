...
const BootScreen: React.FC<BootScreenProps> = ({ onComplete }) => {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined' && typeof sessionStorage !== 'undefined' && sessionStorage.getItem("hasBooted")) {
            onComplete();
            return;
        }
            ...
            >
                ...
                <div className={`relative z-10 ${isGlitching ? "animate-pulse" : ""}`}>
                    {lines.map((line, index) => (
                        <div key={index} className="leading-relaxed drop-shadow-[0_0_2px_rgba(0,255,0,0.8)]">
                            {line || " "}
                        </div>
                    ))}
                    {/* Blinking cursor */}
                    {!isGlitching && (
                        <span className="inline-block w-2.5 h-5 bg-green-500 animate-pulse mt-1 ml-1"></span>
                    )}
                </div>
                ...
                {isGlitching && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center">
                        <div className="w-full h-1/3 bg-white/10 mix-blend-overlay animate-[ping_0.2s_ease-in-out_infinite]"></div>
                        <div className="absolute left-0 w-full h-2 bg-green-400/50 mix-blend-screen animate-[bounce_0.1s_infinite]"></div>
                        <div className="absolute right-0 w-full h-4 bg-purple-500/30 mix-blend-color-dodge animate-[pulse_0.15s_infinite]"></div>
                    </div>
                )}
            </motion.div>
        </AnimatePresence>
